...
class HelpdeskTicket(models.Model):
    ...
    _description = "Ticket de Mesa de Ayuda"
    _rec_name = "number"
    ...
    user_ids = fields.Many2many(
        comodel_name="res.users", related="team_id.user_ids", string="Usuarios"
    )
    ...
    tag_ids = fields.Many2many(comodel_name="helpdesk.ticket.tag", string="Etiquetas")
    ...
    channel_id = fields.Many2one(
        ...
        string="Canal",
        help="El canal indica de dónde proviene el ticket"
        " (puede ser una llamada telefónica, un correo electrónico...)",
    )
    ...
    sequence = fields.Integer(
        index=True,
        default=10,
        help="Define el orden de secuencia al mostrar una lista de tickets.",
    )
    active = fields.Boolean(default=True)

    duplicate_id = fields.Many2one(
        "helpdesk.ticket", string="Duplicado de", tracking=True, copy=False
    )
    duplicate_ids = fields.One2many(
        "helpdesk.ticket", "duplicate_id", string="Tickets duplicados"
    )
