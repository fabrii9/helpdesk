...
class HelpdeskTeam(models.Model):
    ...
    _description = "Equipo de Ticket de Mesa de Ayuda"
    ...
    category_ids = fields.Many2many(
        comodel_name="helpdesk.ticket.category", string="Categoría"
    )
    ...
    alias_id = fields.Many2one(
        ...
        ondelete="restrict",
        required=True,
        help="La dirección de correo electrónico asociada con \
                               este canal. Los nuevos correos recibidos \
                               crearán automáticamente nuevos tickets asignados \
                               al canal.",
    )
    ...
    todo_ticket_count = fields.Integer(
        string="Número de tickets", compute="_compute_todo_tickets"
    )
    todo_ticket_count_unassigned = fields.Integer(
        string="Número de tickets sin asignar", compute="_compute_todo_tickets"
    )
    todo_ticket_count_unattended = fields.Integer(
        string="Número de tickets sin atender", compute="_compute_todo_tickets"
    )
    todo_ticket_count_high_priority = fields.Integer(
        string="Número de tickets en alta prioridad", compute="_compute_todo_tickets"
    )
    show_in_portal = fields.Boolean(
        string="Mostrar en formulario del portal",
        default=True,
        help="Permitir seleccionar este equipo al crear un nuevo ticket en el portal.",
    )
    parent_id = fields.Many2one(
        "helpdesk.ticket.team", string="Equipo padre", index=True
    )
    ...
    def _retrieve_dashboard(self):
        ...
        return [
            ...
            {
                "name": self.env._("Tickets abiertos"),
