...
class HelpdeskTicketMerge(models.TransientModel):
    ...
    _description = "Fusión de Tickets de Mesa de Ayuda"

    ticket_ids = fields.Many2many(
        "helpdesk.ticket", string="Tickets a fusionar", required=True
    )
    user_id = fields.Many2one("res.users", string="Asignado a")
    create_new_ticket = fields.Boolean(string="Crear un nuevo ticket")
    dst_ticket_name = fields.Char(string="Nombre del nuevo ticket")
    dst_helpdesk_team_id = fields.Many2one(
        "helpdesk.ticket.team", string="Equipo de destino"
    )
    dst_ticket_id = fields.Many2one(
        "helpdesk.ticket", string="Fusionar en un ticket existente"
    )
    ...
    def merge_tickets(self):
        ...
        self._add_message(self.env._("desde"), ticket_numbers, self.dst_ticket_id)
        ...
        return "\n".join(
            tickets.mapped(
                lambda ticket: self.env._(
                    "Descripción del ticket %(name)s: %(description)s"
                )
                % {
                    "name": ticket.name,
                    "description": ticket.description or self.env._("Sin descripción"),
                }
            )
        )
    ...
    def _add_message(self, way, ticket_numbers, ticket):
        ...
        """
        subject = "Fusión de ticket de mesa de ayuda"
        body = Markup(
            self.env._("Este ticket de mesa de ayuda ha sido fusionado %(way)s %(tickets)s")
